#!/usr/bin/env python
""" Generate heuristic optimality plots in Fig. 4
...
for i in range(mean_a.shape[1]): # for over dim factor
    ax1.errorbar(n_all,mean_a[:,i,0],var_a[:,i,0], label=f'$r=$'+str(all_dimension_factors[i])+f'$d$', color=cmap_r(0.25 + 0.25 * i))
    ax2.errorbar(n_all,mean_a[:,i,1],var_a[:,i,1], label=f'$r=$'+str(all_dimension_factors[i])+f'$d$', color=cmap_r(0.25 + 0.25 * i))
